...
class PaymentHelper
{
    ...
    private $flowApiBaseUrl = 'https://www.flow.cl/api';
    // private $flowApiBaseUrl = 'https://sandbox.flow.cl/api';
    ...
    public function createSubscriptionRenewalTransaction($subscription, $paymentSucceeded, $paymentId = null)
    {
        ...
        if ($paymentId != null) {
            ...
            } elseif ($transaction['payment_provider'] === Transaction::FLOW_PROVIDER) {
                $transaction['flow_subscription_id'] = $paymentId;
            }
        }
    ...
    public function generateFlowTransaction($transaction) {
        try {
            $client = new Client();
            $data = $this->createFlowTransactionData($transaction);
    
            $response = $client->post($this->flowApiBaseUrl.'/payment/create', [
                'form_params' => $data
            ]);
            ...
            if (isset($responseBody['token'])) {
                $url = trim($responseBody['url']);
                $token = trim($responseBody['token']);
                $transaction->flow_transaction_token = $responseBody['token'];
                $redirectUrl = $url . '?token=' . urlencode($token);
            }
    
            $transaction->save();
    
        } catch (\Exception $e) {
            Log::channel('payments')->error("Error generating Flow transaction: " . $e->getMessage());
            return null;
        }
    ...
    private function generateFlowUniqueTransactionToken($transaction) {
        do {
            $id = Uuid::uuid4()->getHex();
        } while (Transaction::query()->where('flow_transaction_token', $id)->first() != null);
        
        $transaction->flow_transaction_token = $id;
        return $id;
    }

    public function verifyFlowTransaction($token) {
        $transaction = null;
        try {
            Log::info("Verifying Flow transaction with token: " . $token);
    
            $transaction = Transaction::query()->where('flow_transaction_token', $token)->first();
            Log::info("Transaction found: " . json_encode($transaction));
    
            if ($transaction && $transaction->status !== Transaction::APPROVED_STATUS) {
                $data = [
                    'apiKey' => getSetting('payments.flow_api_key'),
                    'token' => $token,
                ];
    
                $secretKey = getSetting('payments.flow_api_secret');
                $toSign = 'apiKey' . $data['apiKey'] . 'token' . $data['token'];
                $signature = hash_hmac('sha256', $toSign, $secretKey);
    
                $flowUrl = $this->flowApiBaseUrl . '/payment/getStatus?apiKey=' . $data['apiKey'] . '&token=' . $token . '&s=' . $signature;
                Log::info("Flow URL: " . $flowUrl);
    
                $client = new Client();
                $response = $client->get($flowUrl, [
                    'headers' => [
                        'Content-Type' => 'application/json',
                    ]
                ]);
                ...
                $responseBody = json_decode($response->getBody(), true);
                Log::info("Flow API Response: " . json_encode($responseBody));
    
                if (isset($responseBody['status']) && $responseBody['status'] === 2) {
                    $transaction->status = Transaction::APPROVED_STATUS;
                    $transaction->save();
                    Log::info("Transaction approved and saved. Status: " . $transaction->status);
                    ...
                    $transaction->status = Transaction::CANCELED_STATUS;
                    $transaction->save();
                    Log::info("Transaction canceled. Status: " . $transaction->status);
                }
            }
        } catch (\Exception $exception) {
            Log::error("Error verifying Flow transaction: " . $exception->getMessage());
        }
    
        return $transaction;
    }

    private function createFlowTransactionData($transaction) {
        $orderId = (string)self::generateFlowUniqueTransactionToken($transaction);
        $exchangeRate = $this->getExchangeRate();
        $amountInClp = round(floatval($transaction->amount) * floatval($exchangeRate));
        $data = [
            'apiKey' => getSetting('payments.flow_api_key'),
            'commerceOrder' => $orderId,
            'subject' => self::getPaymentDescriptionByTransaction($transaction),
            'amount' => $amountInClp,
            'email' => Auth::user()->email,
            'urlConfirmation' => route('payment.callFlow'),
            'urlReturn' => route('payment.checkFlowPaymentStatus'),
            'currency' => 'CLP',
        ];
    ...
    public function generateFlowTransactionWithSubscription($transaction) {
        try {
            $client = new Client();
            $data = $this->createFlowTransactionData($transaction);
            ...
            if ($existingSubscription != null) {
                ...
                $subscription = $this->createSubscriptionFromTransaction($transaction);
                $subscription['amount'] = $transaction['amount'];
                $subscription['flow_subscription_id'] = $transaction['flow_transaction_token'];
                $expiresDate = new DateTime('+'.PaymentsServiceProvider::getSubscriptionMonthlyIntervalByTransactionType($transaction->type).' month', new DateTimeZone('UTC'));
                $subscription['expires_at'] = $expiresDate;
                $subscription->save();
            }
            ...
            Log::channel('payments')->error("Error generating Flow transaction with subscription: " . $e->getMessage());
            return null;
        }
    ...
    private function getExchangeRate() {
        try {
            $client = new Client();
            $response = $client->get('https://cl.dolarapi.com/v1/cotizaciones/usd');
            ...
            Log::error("Error obtaining exchange rate: " . $e->getMessage());
        }
