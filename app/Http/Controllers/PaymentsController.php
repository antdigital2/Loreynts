...
class PaymentsController extends Controller
{
            ...
            switch ($transactionType) {
                ...
                case Transaction::MESSAGE_UNLOCK:
                    ...
                    if ($transaction['payment_provider'] == Transaction::PAYPAL_PROVIDER) {
                        ...
                        $redirectLink = $this->paymentHandler->generateFlowTransaction($transaction);
                    }
                    break;
                ...
                case Transaction::YEARLY_SUBSCRIPTION:
                    ...
                    if ($transaction['payment_provider'] == Transaction::PAYPAL_PROVIDER) {
                        ...
                        $redirectLink = $this->paymentHandler->generateFlowTransactionWithSubscription($transaction);
                    }                    
                    break;
                default:
    ...
    public function mercadoHook(Request $request) {
        ...
        /**
     * Verifies Flow transaction.
     * @param Request $request
     * @return \Illuminate\Http\RedirectResponse
     */
    public function verifyFlowTransaction(Request $request){
        // Capture the token from the request
        $token = $request->input('token'); // Or use $request->get('token');
        Log::info("Token captured", ['token' => $token]);
    
        $transaction = $this->paymentHandler->verifyFlowTransaction($token);
     ...
     * @return void
     */
    public function flowHook(Request $request)
    {
        $content = json_decode($request->getContent(), true);
        Log::channel('payments')->debug("Flow hook received: ", [$content]);

        if (isset($content['data']) && isset($content['data']['id']) && isset($content['action'])) {
            switch ($content['action']) {
                case 'payment.created':
                case 'payment.updated':
                    $this->paymentHandler->verifyFlowTransaction($content['data']['id']);
                    break;
            }
        }
    ...
    public function handleFlowPaymentCallback(Request $request)
    {
        ...
        if ($transaction) {
            try {
                $client = new Client();
                $response = $client->get('https://sandbox.flow.cl/api/payment/getStatus', [
                    'headers' => [
                ...
                Log::channel('payments')->error("Error verifying Flow payment: " . $e->getMessage());
            }
        }
    }
